...
__all__ = ["TermTime", "TermTimeTS", "getTT", "getTTByHour"]
    ...
    def __init__(self, day="tomorrow", time="7am"):
        ...
        if day.lower() == "tomorrow":
            ...
            try:
                ...
                raise ValueError(f"Could not get date from [{day}] ({error})")
        ...
        try:
            ...
            self.ts = None
            raise ValueError(f"Could not set terminate time for [{dayStr}/{time}]=[{timeStr}] ({error})")
...
def getTTByHour():
    endTime = Timestamp.today() + timedelta(hours=1)
    tt = TermTime(endTime=endTime)
    return tt
