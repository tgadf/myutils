...
class FSPath:
    ...
    def touchFile(self, debug=True):
        if self.access():
            if not self.path.is_file():
                if debug:
                    print(f"  ==> Touching File: {self.path}")
                self.path.touch()
    ...
    def mvFile(self, dstPath, debug=True):
        if self.access():
            ...
            if debug:
                print("  ==> Moving {0} to {1}".format(self.path, dstPath))
    ...
    def cpFile(self, dstPath, debug=True):
        if self.access():
            ...
            if not dstPath.exists():
                dstPath.touch()
            if debug:
                print(f"  ==> Copying {self.path} to {dstPath}".format(self.path, dstPath))
    ...
    def rmDir(self, debug=True):
        if self.access():
            if debug:
                print(f"  ==> Removing Directory: {self.path}")
            rmtree(self.path)
    ...
    def mvDir(self, dstPath, debug=True):
        if self.access():
            ...
            if debug:
                print(f"  ==> Moving {self.path} to {dstPath}")
            move(self.path, dstPath)
                        
    def glob(self, pattern="*", lazy=True, debug=True):
        if self.access():
            if debug:
                print(f"Running glob({self.path}/{pattern})")
...
class FSBase:
    ...
    def getJoinStr(self, joinValue):
        try:
            ...
            raise TypeError(f"Trying to join path with something [{joinValue}] that can't be a string ({error})")
        return retval
